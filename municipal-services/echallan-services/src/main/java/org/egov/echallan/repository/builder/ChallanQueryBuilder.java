   ...
   /* private static final String QUERY = "SELECT challan.*,chaladdr.*,challan.id as challan_id,challan.tenantid as challan_tenantId,challan.lastModifiedTime as " +
    ...
    private String addPaginationWrapper(String query,List<Object> preparedStmtList,
       ...
       String string = addOrderByClause(criteria);
		...
		finalQuery = finalQuery.replace("{amount}", "(select nullif(sum(bi.amount),0) from egcl_billdetial bi join egcl_bill b on(b.id=bi.billid) where b.businessservice = challan.businessservice and b.consumercode = challan.challanno  and b.status='ACTIVE' group by b.consumercode) as totalamount,");
