...
const DocumentDetails = ({ location, match, history }) => {
  ...
  function onActionSelect(action) {
    ...
    if (action?.includes("EDIT")) {
      ...
      history.push({
        pathname: `/mgramseva-digit-ui/employee/engagement/documents/inbox/update`,
  ...
  function onModalSubmit() {
    ...
    history.push({
      pathname: `/mgramseva-digit-ui/employee/engagement/documents/delete-response`,
      state: { DocumentEntity },
    });
  }
      ...
      <div className="notice_and_circular_main gap-ten">
        <div className="documentDetails_wrapper">
          <div className="documentDetails_row_items">
            <p className="documentDetails_title">{`${t("ULB")}:`}</p> <p>{getUlbName(details?.tenantId)}</p>{" "}
          </div>
          <div className="documentDetails_row_items">
            <p className="documentDetails_title">{`${t("DOCUMENT_NAME")}:`}</p> <p>{details?.name}</p>{" "}
          </div>
          <div className="documentDetails_row_items">
            <p className="documentDetails_title">{`${t("DOCUMENT_CATEGORY")}:`}</p> <p>{t(details?.category)}</p>{" "}
          </div>
          <div className="documentDetails_row_items">
            <p className="documentDetails_title">{`${t("DCOUMENT_DESCRIPTION")}:`}</p>{" "}
            <p className="documentDetails__description">{details?.description?.length ? details?.description : "NA"}</p>{" "}
          </div>
