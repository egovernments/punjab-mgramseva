...
const getDocumentDetails = (value = "", link, t) => (
...
const getDocumentCell = (name = "mSeva", link, docLink, t) => (
  <span className="document-table-docs-columns">
    {link?.length ? (
      <span className="link" onClick={() => openUploadedDocument(link, name)}>
        {" "}
        {t("CE_DOCUMENT_VIEW_LINK")}{" "}
      </span>
    ) : null}
    {docLink.length ? (
      <span className="link" onClick={() => openDocumentLink(docLink, name)}>
        {" "}
  ...
  if (isLoading) {
    ...
    result = (
      <Card style={{ marginTop: 20, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        {t("CE_DOCUMENTS_NOT_FOUND")}
        <br />
        <Link className="link" to={`/mgramseva-digit-ui/employee/engagement/documents/inbox/new-doc`}>
          {t("NEW_DOCUMENT_TEXT")}
        </Link>
      </Card>
    );
