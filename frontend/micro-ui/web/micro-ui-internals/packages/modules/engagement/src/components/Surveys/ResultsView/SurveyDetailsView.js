    ...
    <div className="surveydetailsform-wrapper">
      <span className="surveyformfield">
        ...
        <div style={{ display: "grid", gridAutoFlow: "row" }}>
          <Dropdown allowMultiselect={true} optionKey={"i18nKey"} option={userUlbs} selected={selectedTenat} keepNull={true} disable={true} t={t} />
          <RemoveableTag key={"tag"} text={t(userUlbs[0].i18nKey)} extraStyles={{ tagStyles: { display: "flex" } }} />
        </div>
        {/* <button
          type={"button"}
          style={{ border: "2px solid #F47738", backgroundColor: "white", padding: "8px 8px",width:"200px",marginLeft:"50px",marginTop:"-45px" }}
          onClick={() => history.push(`/mgramseva-digit-ui/employee/engagement/surveys/inbox/details/${surveyId}`)}
        >
          <header style={{ color:"#F47738"}}>{t("SURVEY_QUESTIONS")}</header>
        </button> */}
        <LinkLabel onClick={() => history.push(`/mgramseva-digit-ui/employee/engagement/surveys/inbox/details/${surveyId}`)}>
      ...
      <span className="surveyformfield">
        ...
        <TextInput name="title" type="text" disable={true} value={surveyTitle} />
      </span>
      <span className="surveyformfield">
        <label>{t("CS_SURVEY_DESCRIPTION")}</label>
        <TextInput name="description" type="text" disable={true} value={surveyDesc} />
      </span>
    </div>
  );
};
