...
const Inbox = () => {
  ...
  const { data: inboxData, isLoading: inboxLoading } = Digit.Hooks.events.useInbox(
    tenantId,
    {
      // limit: pageSize,
      // offset: pageOffset,
  ...
  const GetStatusCell = (value) =>
    value === "Active" ? <span className="sla-cell-success">{value}</span> : <span className="sla-cell-error">{value}</span>;
  ...
  return inboxLoading ? (
    ...
    <InboxComposer {...{ PropsForInboxLinks, ...propsForSearchForm, ...propsForFilterForm, propsForInboxTable }}>
      {/* <InboxPageLinks /> */}
      {/* <InboxSearchFields />
        <InboxFilterFields />
        <InboxTable /> */}
