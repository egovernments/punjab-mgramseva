...
const SurveyDetails = ({ location, match }) => {
    ...
    try {
      ...
      Digit.Surveys.search(filters).then((ob) => {
        if (ob?.Surveys?.length > 0 && data?.title !== surveyData?.title) {
          ...
          history.push("/mgramseva-digit-ui/employee/engagement/surveys/update-response", details);
        }
      });
    } catch (error) {}
  };
  ...
  const handleDelete = () => {
    ...
    history.push("/mgramseva-digit-ui/employee/engagement/surveys/delete-response", details);
  };
