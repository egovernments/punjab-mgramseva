...
const GroupBills = ({ tenantId, onSubmit, data, count, isLoading, resultOk, serviceType, locality }) => {
  ...
  const handleBillLinkClick = (row) => {
    history.push(
      `/mgramseva-digit-ui/employee/bills/bill-details?connectionNumber=${row?.consumerCode}&tenantId=${tenantId}&service=${row?.businessService}&from=ABG_CANCEL_BILL`,
      row
    );
  };
    ...
    return (
      <div>
        <span className="link">
          <button
            onClick={() => {
              handleBillLinkClick(row?.original);
            }}
          >
  ...
  const handleCancelBillAction = (_data) => {
    ...
    history.push("/mgramseva-digit-ui/employee/bills/response-cancelBill", { filters, currentBill });
    ...
    return (
      <div>
        <span className="link">
          <button
            onClick={() => {
              handleCancelActionClick(row?.original);
            }}
          >
            {t(`${"ABG_CANCEL_BILL"}`)}{" "}
          </button>
  ...
  const startWSBillDownloadJob = async (isConsolidated, key = "ws-bill") => {
    const result = await Digit.WSService.wnsGroupBill({
      key: key ? key : "ws-bill",
      tenantId,
      locality: locality?.code,
      isConsolidated,
      bussinessService: serviceType,
    });
    setShowToast({
  ...
  return (
    <>
      <div className="custom-group-merge-container employee-application-details">
        <Header styles={{ fontSize: "32px" }}>{t("ABG_COMMON_HEADER")}</Header>
        {data && data?.length >= 0 && (
          <MultiLink
