...
const BillDetailsv1 = (props) => {
  ...
  const result = Digit.Hooks.useFetchBillsForBuissnessService({ businessService: serviceType, ...{ consumerCode: connectionNumber }, tenantId });
  ...
  const { isLoading, isError, data: applicationDetails, error } = Digit.Hooks.ws.useConnectionDetail(
    t,
    tenantId,
    connectionNumber,
    serviceType === "WS" ? "WATER" : "SEWERAGE"
  );
  ...
  const handleCancelBillAction = (_data) => {
    ...
    history.push("/mgramseva-digit-ui/employee/bills/response-cancelBill", { filters, bill });
  };
