    ...
    if (action) {
      if (action?.isToast) {
        ...
        if (action?.redirectionUrll?.action === "ACTIVATE_CONNECTION") {
          ...
          window.location.assign(`${window.location.origin}/mgramseva-digit-ui/employee/payment/collect/${action?.redirectionUrll?.pathname}`);
        }
      ...
      mutate(data, {
        ...
        onSuccess: (data, variables) => {
          ...
          if (isOBPS?.bpa) {
            ...
            history.replace(`/mgramseva-digit-ui/employee/obps/response`, { data: data });
          }
          if (isOBPS?.isStakeholder) {
            data.selectedAction = selectedAction;
            history.push(`/mgramseva-digit-ui/employee/obps/stakeholder-response`, { data: data });
          }
          if (isOBPS?.isNoc) {
            history.push(`/mgramseva-digit-ui/employee/noc/response`, { data: data });
          }
          if (data?.Amendments?.length > 0) {
            //RAIN-6981 instead just show a toast here with appropriate message
            //show toast here and return
            //history.push("/mgramseva-digit-ui/employee/ws/response-bill-amend", { status: true, state: data?.Amendments?.[0] })
