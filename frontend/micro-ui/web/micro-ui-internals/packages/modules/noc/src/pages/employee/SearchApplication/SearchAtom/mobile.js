...
const SearchApplicationMobileView = ({ SearchFormFieldsComponent, propsMobileInboxCards, isLoading, data, getRedirectionLink, onSubmit }) => {
  ...
  const CurrentMobileModalComponent = useCallback(
    ({ currentlyActiveMobileModal, ...props }) => MobileComponentDirectory({ currentlyActiveMobileModal, ...props }),
      ...
      {data?.[0]?.display ? (
        ...
        <DetailsCard
          {...{
            ...
            linkPrefix: `/mgramseva-digit-ui/employee/noc/search/application-overview/`,
