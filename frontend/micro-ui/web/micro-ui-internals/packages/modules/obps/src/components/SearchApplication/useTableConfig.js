    ...
    () => [
                ...
                <Link
                  to={
                    window.location.href.includes("/citizen")
                      ? `/digit-ui/citizen/obps/${getRedirectionLink(row.original["businessService"]) || "--"}/${
                          row.original["applicationNo"] || row.original["applicationNumber"]
                        }`
                      : `/mgramseva-digit-ui/employee/obps/${getRedirectionLink(row.original["businessService"]) || "--"}/${
                          row.original["applicationNo"] || row.original["applicationNumber"]
                        }`
                  }
                >
          ...
          return (
            <div>
              <span className="cell-text">
                {row.original?.additionalDetails?.applicationType
                  ? t(`WF_BPA_${row.original?.additionalDetails?.applicationType}`)
                  : row.original?.businessService
                  ? t(`BPA_APPLICATIONTYPE_${row?.original?.businessService}`)
                  : "-"}
              </span>
            </div>
          );
      ...
      },
      {
        ...
        accessor: (row) =>
          GetCell(
            t(
              row.additionalDetails?.serviceType ||
                t(`TRADELICENSE_TRADETYPE_${row?.tradeLicenseDetail?.tradeUnits?.[0]?.tradeType?.split(".")[0]}`) ||
                "-"
            )
          ),
      },
