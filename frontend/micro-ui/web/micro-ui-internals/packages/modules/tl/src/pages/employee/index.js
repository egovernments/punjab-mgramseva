...
const TLBreadCrumb = ({ location }) => {
  ...
  const crumbs = [
    ...
    {
      path: "/mgramseva-digit-ui/employee/tl/inbox",
    ...
    },
    {
      path: sessionStorage.getItem("applicationNumber")
        ? `/mgramseva-digit-ui/employee/tl/application-details/${sessionStorage.getItem("applicationNumber")}`
        : "",
    ...
    {
      path: "/mgramseva-digit-ui/employee/tl/new-application",
      ...
      <React.Fragment>
        <div className="ground-container" style={locationCheck ? { width: "100%", marginLeft: "0px" } : { marginLeft: "0px" }}>
          ...
          {/* <p className="breadcrumb" style={{ marginLeft: mobileView ? "2vw" : !locationCheck ? "revert": "15px" }}>
            ...
            / 
            {location.pathname === "/mgramseva-digit-ui/employee/tl/inbox" ? 
              <span>{location.pathname === "/mgramseva-digit-ui/employee/tl/inbox" ? t("ES_COMMON_INBOX") : ""}</span>
              : 
              <Link to="/mgramseva-digit-ui/employee/tl/inbox" style={{ cursor: "pointer", color: "#666" }}>
                {location.pathname.includes("/mgramseva-digit-ui/employee/tl/") ? t("ES_COMMON_INBOX") : ""}
              </Link>}
            <span>{location.pathname.includes("/mgramseva-digit-ui/employee/tl/search/application") ? `/ ${t("ES_COMMON_SEARCH_APPLICATION") }`  : null}</span>
            <span>{location.pathname.includes("/mgramseva-digit-ui/employee/tl/search/license") ? `/ ${t("TL_SEARCH_TRADE_HEADER") }`  : null}</span>
            <span>{location.pathname.includes("/mgramseva-digit-ui/employee/tl/application-details") ? `/ ${t("TL_DETAILS_HEADER_LABEL") }`  : null}</span>
            <span>{location.pathname.includes("/mgramseva-digit-ui/employee/tl/new-application") ? `/ ${t("TL_HOME_SEARCH_RESULTS_NEW_APP_BUTTON") }`  : null}</span>
            <span>{location.pathname.includes("/mgramseva-digit-ui/employee/tl/renew-application-details") ? `/ ${t("ES_TITLE_RENEW_TRADE_LICESE_APPLICATION") }`  : null}</span>
            <span>{location.pathname.includes("/mgramseva-digit-ui/employee/tl/edit-application-details") ? `/ ${t("ES_TITLE_RE_NEW_TRADE_LICESE_APPLICATION") }`  : null}</span>
          </p> */}
          <PrivateRoute
            path={`${path}/inbox`}
