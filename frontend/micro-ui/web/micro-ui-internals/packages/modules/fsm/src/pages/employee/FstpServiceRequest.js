...
const FstpServiceRequest = () => {
  ...
  //     history.push('/mgramseva-digit-ui/employee/fsm/fstp/new-vehicle-entry/')
  // }
  ...
  if (vehicleLog?.length === 0 && tripDetails?.length === 0 && isSuccess && !isSearchLoading && tripDetail?.length === 0 && !isVehiclesLoading) {
    history.push(`/mgramseva-digit-ui/employee/fsm/fstp/new-vehicle-entry/${vehicleNumber}`);
  }
    ...
    return (
      <div>
        ...
        <MobileInbox
          ...
          linkPrefix={"/mgramseva-digit-ui/employee/fsm/fstp-operator-details/"}
          onSearch={onSearch}
        ...
        <span className="link" style={{ margin: "8px" }}>
          <Link
            to={{
              pathname: "/mgramseva-digit-ui/employee/fsm/fstp/new-vehicle-entry/",
            }}
          >
