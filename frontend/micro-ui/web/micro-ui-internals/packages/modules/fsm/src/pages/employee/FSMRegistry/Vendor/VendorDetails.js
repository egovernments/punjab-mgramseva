...
const VendorDetails = (props) => {
  ...
  useEffect(() => {
    switch (selectedAction) {
      ...
      case "EDIT":
        return history.push("/mgramseva-digit-ui/employee/fsm/registry/modify-vendor/" + dsoId);
      default:
        break;
    }
  }, [selectedAction]);
  ...
  const handleVendorUpdate = () => {
    ...
    mutate(formData, {
      ...
      onSuccess: (data, variables) => {
        ...
        setTimeout(() => {
          closeToast();
          if (selectedAction === "DELETE") history.push(`/mgramseva-digit-ui/employee/fsm/registry`);
        }, 5000);
      },
    });
    setShowModal(false);
  ...
  const onEdit = (details, type, id) => {
    if (type === "ES_FSM_REGISTRY_DETAILS_TYPE_DRIVER") {
      history.push("/mgramseva-digit-ui/employee/fsm/registry/modify-driver/" + id);
    } else {
