...
const SearchPTID = ({ tenantId, t, payload, showToast, setShowToast, ptSearchConfig }) => {
  ...
  const handleCollectTaxClick = (val) => {
    ...
    if (isAtleastOneMobileNumberInvalid) {
      ...
      revalidate();
      history.push(`/mgramseva-digit-ui/employee/payment/collect/PT/${val?.["propertyId"]}`);
    }
  };
  ...
  const skipNContinue = () => {
    history.push(`/mgramseva-digit-ui/employee/payment/collect/PT/${selectedProperty?.["propertyId"]}`);
  };
    ...
    () => [
      {
        ...
        Cell: ({ row }) => {
          return (
            <div>
              <span className="link">
                <Link to={`/mgramseva-digit-ui/employee/pt/ptsearch/property-details/${row.original["propertyId"]}`}>
