...
const ApplicationBillAmendment = () => {
  ...
  const rebateAndPenaltyTaxHeads = data?.BillingService?.TaxHeadMaster?.filter(
    (e) => e.code.includes("ADHOC") && e.IsBillamend && e.service === servicev1
  );
  ...
  const uploadFile = async (data, fname, props) => {
    ...
    try {
      if (data?.target?.files?.[0]?.size < 5242880 && /(.*?)(jpg|jpeg|png|pdf)$/i.test(data?.target?.files?.[0]?.type)) {
        ...
        } else setUploadError({ message: "FILE_TYPE_MISMATCHED", documentType: fname });
      }
    } catch (err) {
    } finally {
  ...
  const getDemandDetailsComparingUpdates = (d) => {
    let actionPerformed;
    const action =
      (d?.[`${servicev1}_REDUCED_AMOUNT`]?.VALUE !== undefined && d?.[`${servicev1}_REDUCED_AMOUNT`]?.VALUE) ||
      (d?.[`${servicev1}_ADDITIONAL_AMOUNT`]?.VALUE !== undefined && !d?.[`${servicev1}_ADDITIONAL_AMOUNT`]?.VALUE)
        ? "rebate"
        : "penalty";
    if (servicev1 === "WS")
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (goToAppDetailsPage)
        window.location.href = `${window.location.origin}/mgramseva-digit-ui/employee/ws/generate-note-bill-amendment?applicationNumber=${state?.data.applicationDetails.amendment.amendmentId}`;
    }, 5000);
  ...
  const onFormSubmit = (d) => {
        ...
        { AmendmentUpdate },
        {
          ...
          onSuccess: (data, variables) => {
            ...
            if (data?.Amendments?.length > 0) {
              ...
              // history?.push(`/mgramseva-digit-ui/employee/ws/application-details-bill-amendment?applicationNumber=${data?.Amendments[0].amendmentId}`)
              return;
            }
          },
        }
    ...
    history.push("/mgramseva-digit-ui/employee/ws/response", data);
  };
  ...
  function getReasonDocNoHeader() {
    if (amendmentReason?.code === "COURT_CASE_SETTLEMENT") return "WS_COURT_ORDER_NO";
  ...
  let adhocAmount = 0;
  billSearchData
    ?.filter((ob) => ob?.taxHeadCode?.includes("ADHOC"))
    ?.map((ob) => {
      ...
      <Card>
          ...
          <table cellPadding={"8px"} cellSpacing={"10px"}>
            <div style={isMobile && isEmployee ? { overflow: "scroll", maxWidth: "50%" } : {}}>
              <tbody>
                <tr style={{ textAlign: "left" }}>
                          ...
                          return (
                            <div style={{ paddingTop: "20px" }}>
                              <CheckBox
                                ...
                                disable={
                                  isEdit &&
                                  (state?.data?.applicationDetails?.amendment?.additionalDetails?.editForm?.["WS_REDUCED_AMOUNT"]?.["VALUE"] ==
                                    false ||
                                    state?.data?.applicationDetails?.amendment?.additionalDetails?.editForm?.["SW_REDUCED_AMOUNT"]?.["VALUE"] ==
                                      false)
                                    ? true
                                    : false
                                }
                  ...
                  <th>
                    <>
                          ...
                          return (
                            <div style={{ paddingTop: "17px" }}>
                              <CheckBox
                                ...
                                disable={
                                  isEdit &&
                                  (state?.data?.applicationDetails?.amendment?.additionalDetails?.editForm?.["WS_ADDITIONAL_AMOUNT"]?.["VALUE"] ==
                                    false ||
                                    state?.data?.applicationDetails?.amendment?.additionalDetails?.editForm?.["WS_ADDITIONAL_AMOUNT"]?.["VALUE"] ==
                                      false)
                                    ? true
                      ...
                      />
                      {errors?.WS_ADDITIONAL_AMOUNT?.VALUE && ischeckedAddition == true ? (
                        <CardLabelError>{t("WS_REQUIRED_FIELD")}</CardLabelError>
                      ) : null}
                    </>
                  </th>
                </tr>
                    ...
                    <div>
                      ...
                      {servicev1 === "WS"
                        ? errors?.WS_REDUCED_AMOUNT?.[node.taxHeadCode] && (
                            <CardLabelError style={{ marginBottom: "-20px", marginTop: "-30px", marginLeft: "150px" }}>
                              {errors?.WS_REDUCED_AMOUNT?.[node.taxHeadCode]?.message}
                            </CardLabelError>
                          )
                        : errors?.SW_REDUCED_AMOUNT?.[node.taxHeadCode] && (
                            <CardLabelError style={{ marginBottom: "-20px", marginTop: "-30px", marginLeft: "150px" }}>
                              {errors?.SW_REDUCED_AMOUNT?.[node.taxHeadCode].message}
                            </CardLabelError>
                          )}
                    </div>
                    <td style={{ paddingRight: "60px" }}>
                      ...
                      {servicev1 === "WS"
                        ? errors?.WS_ADDITIONAL_AMOUNT?.[node.taxHeadCode] && (
                            <CardLabelError style={{ marginTop: "-19px" }}>
                              {errors?.WS_ADDITIONAL_AMOUNT?.[node.taxHeadCode]?.message}
                            </CardLabelError>
                          )
                        : errors?.SW_ADDITIONAL_AMOUNT?.[node.taxHeadCode] && (
                            <CardLabelError style={{ marginTop: "-19px" }}>
                              {errors?.SW_ADDITIONAL_AMOUNT?.[node.taxHeadCode]?.message}
                            </CardLabelError>
                          )}
                    </td>
                  </tr>
                ...
                {
                  <tr>
                    <td style={{ paddingRight: "60px", fontWeight: "700" }}>
                      {t("WS_REBATE_PENALTY")}
        ...
        {BillAmendmentMDMSLoading ? (
          ...
          <>
            ...
            {amendmentReason?.code !== "COURT_CASE_SETTLEMENT" && (
              <div>
                <LabelFieldPair>
                ...
                {errors?.effectiveTill?.type === "required" && (
                  <CardLabelError style={{ width: "70%", marginLeft: "30%", fontSize: "12px", marginTop: "-21px" }}>
                    {t("WS_REQUIRED_FIELD")}
                  </CardLabelError>
                )}
                {errors?.effectiveTill?.message === "Invalid format" && (
                  <CardLabelError style={{ width: "70%", marginLeft: "30%", fontSize: "12px", marginTop: "-21px" }}>
        ...
        {requiredDocuments?.map((e) => (
          <div style={{ marginBottom: "25px" }}>
            ...
            {errors?.["DOCUMENTS"]?.[e?.documentType] || (error && error?.documentType === e.documentType) ? (
              <CardLabelError style={{ width: "70%", marginLeft: "30%", fontSize: "12px" }}>
                {error && error?.documentType === e.documentType ? t(error?.message) : t("WS_NO_FILE_SELECTED")}
              </CardLabelError>
            ) : null}
          </div>
        ))}
