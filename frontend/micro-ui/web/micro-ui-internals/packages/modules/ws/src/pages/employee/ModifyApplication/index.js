...
const ModifyApplication = () => {
  ...
  useEffect(() => {
    if (!isConfigLoading) {
      ...
      config?.body?.forEach((data) => {
        if (data?.isModifyConnection) bodyDetails.push(data);
      });
      bodyDetails.forEach((bdyData) => {
        if (bdyData?.head == "WS_COMMON_PROPERTY_DETAILS") bdyData.head = "";
      });
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (isAppDetailsPage)
        window.location.href = `${window.location.origin}/mgramseva-digit-ui/employee/ws/application-details?applicationNumber=${
