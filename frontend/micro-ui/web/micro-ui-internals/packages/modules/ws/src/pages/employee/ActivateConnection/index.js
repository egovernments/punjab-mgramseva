...
const ActivateConnection = () => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (isAppDetailsPage)
        window.location.href = `${window.location.origin}/mgramseva-digit-ui/employee/ws/application-details?applicationNumber=${filters?.applicationNumber}&service=${filters?.service}`;
    }, 3000);
  ...
  const closeToast = () => {
    setShowToast(null);
    // history.push(`/mgramseva-digit-ui/employee/ws/application-details?applicationNumber=${filters?.applicationNumber}&service=${filters?.service}`, {});
  };
