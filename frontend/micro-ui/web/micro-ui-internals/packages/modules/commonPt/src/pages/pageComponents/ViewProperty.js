...
const ViewProperty = () => {
  ...
  const onSubmit = () => {
    ...
    return history.push(
      `${redirectToUrl}?propertyId=${propertyId}&tenantId=${applicationDetails?.tenantId || tenantId}${
        state?.data?.applicationDetails?.applicationStatus === "PENDING_FOR_FIELD_INSPECTION"
          ? `&applicationNumber=${state?.data?.applicationDetails?.applicationNo}&service=${state?.data?.applicationDetails?.applicationData?.serviceType}`
          : ``
      }`,
      { ...state }
    );
  ...
  useEffect(() => {
    if (sessionStorage.getItem("isCreateEnabledEmployee") === "true") {
      ...
      history.replace("/mgramseva-digit-ui/employee");
